...
export default function Terminal({ logs, status }: Props) {
  ...
  return (
    <section className="overflow-hidden rounded-xl border border-term-line bg-term shadow-[0_18px_40px_-22px_rgba(12,23,18,0.55)]">
      <div className="flex items-center gap-2 border-b border-term-line px-4 py-2.5">
        ...
        <span className="ml-2 font-mono text-[11px] tracking-[0.1em] text-[#7e9a88]">
      ...
      >
        {logs.length === 0 && (
          <p className="text-[#5c7566]">$ offradar --source alrajhi --watch</p>
        )}
        {logs.map((l) => (
          <p key={l.id} className={COLOR[l.kind]}>
            <span className="text-[#55705f]">[{l.time}]</span>{" "}
            {l.kind === "err" ? "✗ " : l.kind === "ok" ? "✓ " : l.kind === "warn" ? "! " : l.kind === "sys" ? "» " : "· "}
            {l.text}
          </p>
        ))}
        {status === "running" && (
          <span className="caret inline-block h-[13px] w-[7px] translate-y-[2px] bg-lime" />
        )}
      </div>
    </section>
  );
